...
module.exports = class Api extends EventEmitter {
	...
	handleHttpAnimate(frames, with_repeat) {
		...
		for (let i = 0; i < frames.length; i++) {
			...
			// Read 'delay' property
			if (!frames[i].hasOwnProperty("delay"))
				return "Missing 'delay' property in frame #" + i + ".";
			if ((animationFrame.delay = parseFloat(frames[i].delay)) === NaN || animationFrame.delay < 0)
				return "Value of 'delay' property in frame #" + i + " must be a non-negative float.";
